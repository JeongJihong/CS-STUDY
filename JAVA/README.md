...
:pencil2: [**객체지향과 절차지향의 차이**](#pencil2-객체지향과-절차지향의-차이)

:pencil2: [**RESTful API**](#pencil2-RESTful-API)
<br><br>
...
1. Annotation이란?
   ...
   - 어노테이션의 용도는 다양한 목적이 있지만 메타 데이터의 비중이 가장 큼
      - 메타 데이터란? 데이터에 대한 설명을 의미하는 데이터 (자신의 정보를 담고 있는 데이터)
...
- 클래스 내부에서 지정하는 것이 아닌 외부에서 사용자에 의해 지정되는 것을 의미한다. 한마디로 특정(Specific) 타입을 미리 지정해주는 것이 아닌 필요에 의해 지정할 수 있도록 하는 일반(Generic) 타입
- Generic의 장점
...
## :pencil2: **객체지향과 절차지향의 차이**
- 객체지향은 프로그래밍에서 필요한 데이터와 동작을 객체로 만들어 구현하는 기법입니다.
- 절차지향은 프로시저 호출의 개념을 바탕으로 하는 구조적 기법입니다.
